...
#include <unistd.h>
#include "backend/npu/impl/core/NPUQueue.h"
...
#include "backend/npu/impl/framework/OpCmdHelper.h"
#include "backend/npu/impl/framework/OpParamMaker.h"
#include "backend/npu/impl/framework/aoe/AoeUtils.h"
...
int ExecFunc(c10_npu::queue::QueueParas* in, aclrtStream stream) {
  ...
  if (cur_paras->customHandler) {
    ...
    if (ret != ACL_ERROR_NONE) {
      ASCEND_LOGE(
          "Custom hand fail! name=%s, ret=0x%#x", cur_paras->opType, ret);
      C10_NPU_SHOW_ERR_MSG();
    }
    return ret;
  }
