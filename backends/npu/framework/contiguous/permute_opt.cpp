...
class PermuteContiguousOpt : public ContiguousOpt {
 public:
  bool Optimizer(
    ...
    if (can_use_permute(src_desc, perm, sizes)) {
      RECORD_FUNCTION(
          "contiguous_d_Transpose", std::vector<c10::IValue>({src}));
  ...
  bool CachedOptimizer(
    ...
    if (src_desc.cached_contiguous) {
      RECORD_FUNCTION(
          "cached_contiguous_d_Transpose", std::vector<c10::IValue>({src}));
    ...
    if (can_use_permute(src_desc, perm, sizes)) {
      RECORD_FUNCTION(
          "contiguous_d_Transpose", std::vector<c10::IValue>({src}));
      CachedContiguousOpt cached_opt = CachedContiguousOpt{"permute"};
 ...
 private:
  ...
  bool can_use_permute(
    ...
    // unchanged.
    if (c10::multiply_integers(view_sizes) !=
        c10::multiply_integers(base_sizes)) {
      return false;
    }
