  ...
  const static ReuseMode reuseMode = []() -> ReuseMode {
    ...
    switch (envFlag) {
      ...
      default:
        TORCH_CHECK(
            false,
            "MULTI_STREAM_MEMORY_REUSE only allow 0, 1, 2",
...
uint32_t OptionsManager::CheckUseHcclAsyncErrorHandleEnable() {
  ...
  int64_t asyncErrorHandlingFlag = (asyncErrorHandling_val != nullptr)
      ? strtol(asyncErrorHandling_val, nullptr, 10)
      : 1;
...
uint32_t OptionsManager::CheckUseDesyncDebugEnable() {
  ...
  int64_t desyncDebugFlag =
      (desyncDebug_val != nullptr) ? strtol(desyncDebug_val, nullptr, 10) : 0;
