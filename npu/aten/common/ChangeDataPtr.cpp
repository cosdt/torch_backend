...
int64_t NPUNativeFunctions::npu_change_data_ptr(
    ...
    int64_t offset) {
  TORCH_CHECK(
      offset >= 0, "Expect offset equal or greater than zero, got: ", offset);
  ...
  if (src_scalar_type == at::ScalarType::Float) {
    float* data_ptr =
        static_cast<float*>(src.storage().data_ptr().get()) + offset;
    aim_data_ptr = at::DataPtr(data_ptr, dst.storage().device());
  } else if (src_scalar_type == at::ScalarType::BFloat16) {
    at::BFloat16* data_ptr =
        static_cast<at::BFloat16*>(src.storage().data_ptr().get()) + offset;
    aim_data_ptr = at::DataPtr(data_ptr, dst.storage().device());
  } else {
    at::Half* data_ptr =
        static_cast<at::Half*>(src.storage().data_ptr().get()) + offset;
    aim_data_ptr = at::DataPtr(data_ptr, dst.storage().device());
  }
