...
#include "backend/npu/NPUTensorImpl.h"
...
at::Tensor NPUNativeFunctions::scalar_tensor(
  ...
  auto result = at::native::empty_cpu(
      {}, dtype, layout, c10::make_optional(c10::Device(at::kCPU)), pin_memory);
...
at::Tensor NPUNativeFunctions::unsafe_empty_with_format(
  ...
  return NPUNativeFunctions::empty_with_format(
      size, dtype_opt, layout_opt, device_opt, pin_memory_opt, dst_format);
}
...
at::Tensor NPUNativeFunctions::bartlett_window(
  ...
  const int64_t first_half_size =
      ((unsigned int64_t)(window_length - 1) >> 1) + 1;
