...
class Stream(torch_backend._C._NPUStreamBase, _StreamBase):
    ...
    def __repr__(self):
        return ('<torch_backend.backend.Stream device={0} npu_stream={1:#x}>'
...
class Event(torch_backend._C._EventBase, _EventBase):
    ...
    def record(self, stream=None):
        ...
        Uses ``torch_backend.backend.current_stream()`` if no stream is specified. The
        stream's device must match the event's device.
        """
        if stream is None:
            stream = torch_backend.backend.current_stream()
    ...
    def wait(self, stream=None):
        ...
        Use ``torch_backend.backend.current_stream()`` if no stream is specified.
        """
        if stream is None:
            stream = torch_backend.backend.current_stream()
    ...
    def __repr__(self):
        if self.npu_event:
            return '<torch_backend.backend.Event {0:#x}>'.format(self._as_parameter_.value)
        else:
            return '<torch_backend.backend.Event uninitialized>'
