...
c10::DeviceType convertDeviceType(const std::string& tpDeviceType) {
  if (tpDeviceType == tensorpipe_npu::kCpuDeviceType) {
    ...
    TORCH_INTERNAL_ASSERT(
        false,
        "Unrecognized TensorPipe buffer type.",
...
std::tuple<tensorpipe_npu::Message, TensorpipeWriteBuffers> tensorpipeSerialize(
  ...
  // kTpMessageTypeIdx = 0
  tpMessage.payloads.push_back(tensorpipe_npu::Message::Payload{
      buffers.type.get(), sizeof(MessageType)});
  // kTpMessageIdIdx = 1
  tpMessage.payloads.push_back(
      tensorpipe_npu::Message::Payload{buffers.id.get(), sizeof(int64_t)});
  ...
  tpMessage.payloads.push_back(
      tensorpipe_npu::Message::Payload{payloadPtr, buffers.payload.size()});
  ...
  for (const auto i : c10::irange(tensorDataVec.size())) {
    ...
    TORCH_INTERNAL_ASSERT(
        tpMessage.tensors.size() == i, DIST_ERROR(ErrCode::INTERNAL));
    c10::optional<std::vector<char>> maybeCopiedTensor =
        converter->prepareTensorForSending(
            tensor.storage(), streams, tpMessage);
    TORCH_INTERNAL_ASSERT(
        tpMessage.tensors.size() == i + 1, DIST_ERROR(ErrCode::INTERNAL));

    tensorpipe_npu::Device targetDevice = devices.empty() || devices[i].is_cpu()
        ? tensorpipe_npu::Device{tensorpipe_npu::kCpuDeviceType, 0}
        : tensorpipe_npu::Device{
              tensorpipe_npu::kNpuDeviceType, devices[i].index()};
...
std::pair<tensorpipe_npu::Allocation, TensorpipeReadBuffers> tensorpipeAllocate(
  ...
  for (const auto tensorIdx : c10::irange(numTensors)) {
    const tensorpipe_npu::Descriptor::Tensor& tensor =
        tpDescriptor.tensors[tensorIdx];
    TORCH_INTERNAL_ASSERT(
        tensor.targetDevice.has_value(), DIST_ERROR(ErrCode::PARAM));
...
c10::intrusive_ptr<Message> tensorpipeDeserialize(
    tensorpipe_npu::Descriptor&& tpDescriptor,
    TensorpipeReadBuffers&& buffers) {
  // Tensors
  ...
  torch::jit::Unpickler unpickler(
      pickleReadFunc, nullptr, nullptr, tensorReadFunc, {}, true);
    ...
    if (tensor.device().type() == c10::DeviceType::PrivateUse1) {
      ...
      c10::intrusive_ptr<c10::StorageImpl> npu_storage_impl =
          c10::make_intrusive<NPUStorageImpl>(
              ...
              storageImpl->sym_nbytes().as_int_unchecked(),
              std::move(storageImpl->mutable_data_ptr()),
              storageImpl->allocator(),
              storageImpl->resizable());
      auto storage = c10::Storage(npu_storage_impl);
      at_npu::native::NPUNativeFunctions::set_(
          tensor, storage, 0, tensor.sizes(), tensor.strides());
      at_npu::native::StorageDescHelper::SetDesc(
          tensor, tensor.sizes(), tensor.strides(), ACL_FORMAT_ND);
    }
  }
  ...
  for (const auto i : c10::irange(tpDescriptor.tensors.size())) {
    ...
    if (tensor.targetDevice.has_value() &&
        tensor.targetDevice->type == tensorpipe_npu::kNpuDeviceType) {
      TORCH_INTERNAL_ASSERT(
