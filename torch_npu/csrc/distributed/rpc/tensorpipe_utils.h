...
class TensorpipeDeviceTypeConverter {
 public:
  // Ideally we'd want this to also return a tensorpipe_npu::Message::Tensor
  // object but we cannot forward-declare that class (because it's nested), and
  // we cannot include the TensorPipe headers because it's a private dependency.
  // Thus we bend over backwards and entrust this method with appending that
  // object to the `tensors` field of the tensorpipe_npu::Message object we
  // pass.
  ...
  // Same as above: this method cannot return a
  // tensorpipe_npu::Allocation::Tensor, thus it appends it to the `tensors`
  // field of the tensorpipe_npu::Allocation.
...
#define C10_REGISTER_TENSORPIPE_DEVICE_TYPE_CONVERTER(                         \
  ...
  static ::torch_npu::distributed::rpc::TensorpipeDeviceTypeConverterRegistrar \
