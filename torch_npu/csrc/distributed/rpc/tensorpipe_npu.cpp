...
std::unique_ptr<ChannelRegistration> makeNpuBasicChannel() {
  auto context = tensorpipe_npu::channel::npu_basic::create(
      tensorpipe_npu::channel::basic::create());
  return std::make_unique<ChannelRegistration>(
      ChannelRegistration{std::move(context), kNpuBasicChannelPriority});
}
...
class TensorpipeNpuConverter : public TensorpipeDeviceTypeConverter {
 public:
  c10::optional<std::vector<char>> prepareTensorForSending(
    ...
    auto stream =
        c10_npu::NPUStream(getStreamForDevice(streams, storage.device()));
