...
class TensorPipeAgent : public RpcAgent {
 ...
 protected:
  ...
  virtual void pipeWrite(
      const std::shared_ptr<tensorpipe_npu::Pipe>&,
      c10::intrusive_ptr<Message> message,
      std::vector<c10::Device>&& devices,
 ...
 private:
  ...
  struct ClientPipe {
    explicit ClientPipe(std::shared_ptr<tensorpipe_npu::Pipe> pipe)
        : pipe_(std::move(pipe)) {}
  ...
  // Local devices used by this agent. If application didn't specify this field,
  // it will be initialized using corresponding local devices in
