...
HcclResult hcclScatter(
    ...
    HcclComm comm,
    aclrtStream stream) {
  typedef HcclResult (*HcclScatterVFunc)(
      void*, void*, uint64_t, HcclDataType, uint32_t, HcclComm, aclrtStream);
...
bool hcclCommInitRootInfoConfigExist() {
  ...
  c10::call_once(flag, [&]() {
    auto func = GET_FUNC(HcclCommInitRootInfoConfig) if (func != nullptr) {
      exist = true;
    }
  });
  return exist;
