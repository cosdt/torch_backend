...
std::string trimPrefix(std::string s) {
  static std::vector<std::string> prefixes =
      py::module::import("torch.profiler.python_tracer")
          .attr("_prefix_regex")()
...
void reportPythonFuncCallDataToNpuProfiler(const RawEvent& event) {
  ProfilerMgr::GetInstance()->Upload(
      std::make_unique<torch_npu::toolkit::profiler::PythonFuncCallData>(
          event.t_,
...
void reportPythonModuleCallDataToNpuProfiler(
    PyObject* mod_class,
    uint64_t idx) {
  ...
  ProfilerMgr::GetInstance()->Upload(
      std::make_unique<torch_npu::toolkit::profiler::PythonModuleCallData>(
          idx,
          torch_npu::toolkit::profiler::Utils::GetTid(),
          torch_npu::toolkit::profiler::Utils::GetPid(),
...
void PythonTracer::start(size_t max_threads) {
  TORCH_CHECK(
      !active_, "PythonTracer is already active", PROF_ERROR(ErrCode::PARAM))
  TORCH_CHECK(
      !trace_contexts_.size(),
  ...
  if (max_threads > 1) {
    ...
    if (thread_states.size() > max_threads) {
      ASCEND_LOGW(
          "Warning: can only trace %zu thread. %zu are currently active.",
          max_threads,
          thread_states.size());
