...
std::vector<uint8_t> OpRangeData::encode() {
  ...
  if (!input_shapes.empty()) {
    encode2DIntegerMatrixDatas<int64_t>(
        static_cast<uint16_t>(OpRangeDataType::INPUT_SHAPE),
        input_shapes,
        result);
  }
  if (!stack.empty()) {
...
std::vector<uint8_t> PythonFuncCallData::encode() {
  ...
  encodeStrData(
      static_cast<uint16_t>(PythonFuncCallDataType::NAME), func_name, result);

  std::vector<uint8_t> resultTLV;
  uint16_t dataType =
      static_cast<uint16_t>(PythonFuncCallDataType::PYTHON_FUNC_CALL_DATA);
  for (size_t i = 0; i < sizeof(uint16_t); ++i) {
    resultTLV.push_back((dataType >> (i * 8)) & 0xff);
  }
  uint32_t length = result.size();
...
std::vector<uint8_t> PythonModuleCallData::encode() {
  ...
  encodeStrData(
      static_cast<uint16_t>(PythonModuleCallDataType::MODULE_UID),
      module_uid,
      result);
  encodeStrData(
      static_cast<uint16_t>(PythonModuleCallDataType::MODULE_NAME),
      module_name,
      result);

  std::vector<uint8_t> resultTLV;
  uint16_t dataType =
      static_cast<uint16_t>(PythonModuleCallDataType::PYTHON_MODULE_CALL_DATA);
  for (size_t i = 0; i < sizeof(uint16_t); ++i) {
    resultTLV.push_back((dataType >> (i * 8)) & 0xff);
  }
  uint32_t length = result.size();
