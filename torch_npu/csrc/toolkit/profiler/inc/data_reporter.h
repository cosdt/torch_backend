...
inline void encodeStrArrayData(
    uint16_t type,
...
inline void encodeMapData(
    uint16_t type,
    const std::unordered_map<std::string, c10::IValue>& datas,
    std::vector<uint8_t>& result) {
  std::string rst;
...
template <typename T>
void encode2DIntegerMatrixDatas(
    uint16_t type,
...
enum class PythonFuncCallDataType { PYTHON_FUNC_CALL_DATA = 1, NAME = 2 };
